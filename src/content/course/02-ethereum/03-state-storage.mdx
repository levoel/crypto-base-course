...
import { StateTrieDiagram } from '../../../components/diagrams/EthereumDiagrams';

# State & Storage

State management — ключевая особенность Ethereum. В отличие от Bitcoin (stateless UTXO), Ethereum хранит полное состояние всех аккаунтов и контрактов. Это stateful модель, где контракт "помнит" значения переменных между вызовами.

Понимание storage layout критично для data engineering: чтобы читать данные напрямую из state (без вызова контракта), нужно знать, в каком слоте хранится нужная переменная. Это основа для создания индексеров, которые эффективно отслеживают изменения состояния.

Storage — самая дорогая часть Ethereum: запись в новый слот стоит 20,000 gas (~$2-5), потому что эти данные хранятся **навсегда** на каждом full node. Это создаёт pressure на разработчиков оптимизировать использование storage.
