...
import { EVMExecutionDiagram, GasModelDiagram, EIP1559FeeDiagram } from '../../../components/diagrams/EthereumDiagrams';

# EVM & Gas

EVM (Ethereum Virtual Machine) — виртуальная машина, которая выполняет smart contracts на каждом узле сети. Это "сердце" Ethereum: когда ты отправляешь транзакцию, каждый full node выполняет тот же код и должен получить идентичный результат. Детерминизм — ключевое свойство EVM.

**Gas** — это единица измерения вычислительной работы. Каждая операция EVM стоит определённое количество gas. Это решает "halting problem": вместо того чтобы определять, завершится ли программа, мы просто ограничиваем ресурсы. Закончился gas — выполнение прерывается.

Для data engineer понимание EVM и gas критично для:
- **Оценки стоимости транзакций** — сколько будет стоить вызов контракта?
- **Декодирования calldata** — какой метод вызывается и с какими параметрами?
- **Анализа gas usage** — почему некоторые транзакции дорогие?
- **Trace analysis** — какие internal calls происходят?
